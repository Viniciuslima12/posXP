...
def extrair_dados_rreo(
    ...
    """
    Executa a extração de dados do rreo para um município específico.
    
    1. Lê a lista de entes de um arquivo parquet (local ou S3).
    2. Filtra pelo município desejado.
    3. Itera pelos períodos, chama a API do rreo e consolida os dados.
    ...
    print(f"Processando rreo para o ano: {ano_referencia}. Total de entes na fila: {len(entes_a_processar)}")
    ...
    for i in entes_a_processar.index:
        ...
        for bimestre in range(1, 7):
            ...
            url = f'{base_url}?an_exercicio={ano_referencia}&id_ente={ente_ibge}&nr_periodo={bimestre}&co_tipo_demonstrativo=RREO'
            ...
            if dados_brutos and dados_brutos.get("items"):
                ...
                print(f"  Bimestre {bimestre}: Recebidos {len(dados_periodo)} registros.")
            else:
                print(f"  Bimestre {bimestre}: Nenhum dado retornado.")
    
    if dados_consolidados_ano.empty:
        print("Atenção: Nenhum dado de rreo foi consolidado para os filtros aplicados.")
